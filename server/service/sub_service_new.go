...
func GetUserSubNew(url string, clientType string) string {
	...
	case "Clash":
		return ClashMeta(&nodeArr)

	case "Shadowrocket":
		return Shadowrocket(&nodeArr)

	case "Surge":
		return Surge(&nodeArr)

	case "Quantumult":
		return Quantumult(&nodeArr)
...
func NekoBox(nodes *[]model.Node) string {
	...
	for _, v := range *nodes {
		...
		case global.NodeTypeVless, global.NodeTypeTrojan, global.NodeTypeHysteria:
			if res := VlessTrojanHysteriaUrl(v); res != "" {
				nodeArr = append(nodeArr, res)
			}
...
func Shadowrocket(nodes *[]model.Node) string {
	...
	for _, v := range *nodes {
		...
		switch v.NodeType {
		case global.NodeTypeVmess, global.NodeTypeVless:
